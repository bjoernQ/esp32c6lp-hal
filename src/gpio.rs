...
#[non_exhaustive]
pub struct Io {
    _peripheral: esp32c6_lp::LP_IO_MUX,
...
impl Io {
    pub fn new(peripheral: esp32c6_lp::LP_IO_MUX) -> Self {
        Self {
...
impl<const PIN: u8, MODE> GpioPin<PIN, MODE> {
    ...
    /// Assuming the GPIO is already configured by the HP core this makes the GPIO into an output pin.
...
impl<const PIN: u8> OutputPin for GpioPin<PIN, Output> {
    ...
    fn set_low(&mut self) -> Result<(), Self::Error> {
        let lp_gpio = unsafe { &*esp32c6_lp::LP_IO_MUX::PTR };
        lp_gpio
            .out_data_w1tc
